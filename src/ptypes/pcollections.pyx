...
cdef inline CSkipNode* getNextAtLevel(Storage storage, CSkipNode *p2CSkipNode, int level) except NULL:
    cdef CSkipNode *x = <CSkipNode* >(storage.baseAddress + (<unsigned long*>(storage.baseAddress+p2CSkipNode.o2Next))[level])
    # print 'getNextAtLevel', hex(<unsigned long><void*>x) 
    return x
cdef inline setNextAtLevel(Storage storage, CSkipNode *p2CSkipNode, int level, CSkipNode* p2):
    # print 'Setting SkipNode', hex(<void*>p2CSkipNode - storage.baseAddress), 'o2Next=', hex(p2CSkipNode.o2Next), 'level=', level, 'p2=', hex(<void*>p2 - storage.baseAddress)
    (<unsigned long *>(storage.baseAddress+p2CSkipNode.o2Next))[level] = <void*>p2 - storage.baseAddress
...
cdef class SkipNodeMeta(PersistentMeta):
    ...
    def __init__(ptype, Storage       storage, 
        ...
        PersistentMeta.__init__(ptype, storage, className, proxyClass, ptype.o2Value + valueClass.assignmentSize )
    ...
    def reduce(ptype):
        assert False, "The name of SkipNodeMeta instances must start with '__' to prevent pickling them!"


cdef class PSkipNode(AssignedByReference):

    cdef inline CSkipNode *getP2IS   (PSkipNode self): return <CSkipNode *>self.p2InternalStructure # XXX inline
    cdef inline void*      getP2Value(PSkipNode self): return self.p2InternalStructure + (<SkipNodeMeta>self.ptype).o2Value # XXX inline

    cdef inline Persistent getValue(self): # XXX inline
        return (<SkipNodeMeta>self.ptype).valueClass.resolveAndCreateProxyFA(self.getP2Value())
    cdef inline setValue(self, value): # XXX inline
        (<SkipNodeMeta>self.ptype).valueClass.assign(self.getP2Value(), value)
    
    cdef inline Persistent getKey(self): # XXX inline
        getKeyFromValue = (<SkipNodeMeta>self.ptype).getKeyFromValue
        return getKeyFromValue(self.getValue()) if getKeyFromValue else self.getValue()
    
    def __init__(PSkipNode self, object value, level=0):
        self.setValue(value)
        self.getP2IS().numberOfLevels = level
        self.getP2IS().o2Next = self.ptype.storage.allocate(level*sizeof(Offset))  # Array of offsets to CSkipNodes
        # print 'new', CSkipNode2str(self.getP2IS(), self.ptype.storage)
...
cdef CSkipNode2str(CSkipNode *node, Storage storage):
    return "[@offset={0} key={1} o2Value={2} o2Next={3} #lvls={4}]"\
        .format(hex(<void*>node-storage.baseAddress), 
                'node.key', 'hex(node.o2Value)', hex(node.o2Next), node.numberOfLevels) # XXX print key properly
...
cdef class SkipListMeta(PersistentMeta):
    ...
    def typedef(       PersistentMeta   meta, 
        ...
        if valueClass is None:
            raise ValueError("The type parameter specifying the type of values cannot be {0}"
                             .format(valueClass))
        cdef:
            str entryName = '__{valueClass.__name__}AsSkipListNode' .format(valueClass=valueClass) 
            SkipNodeMeta entryClass = SkipNodeMeta.typedef(storage, entryName , PSkipNode, valueClass, sortOrder)
            
        return super(SkipListMeta, meta).typedef(storage, className, proxyClass, entryClass)
    ...
    def __init__(ptype, 
        ...
        PersistentMeta.__init__(ptype, storage, className, proxyClass, sizeof(CSkipList) )
...
cdef class PSkipList(AssignedByReference):
    ...
    cdef inline CSkipList *getP2IS(self): return <CSkipList *>self.p2InternalStructure    
    
    def __init__(PSkipList self, ):
        # The head node is never visible outside, so its key and value do not matter.
        # Its only important parts are o2Next (its array of offsets of the next nodes)
        # and numberOfLevels (the size of the array).
        # The skip list cannot have a node with an array bigger than this one.          
        self.getP2IS().o2Head = allocateStorage(self.ptype)
        # (<SkipListMeta>self.ptype).entryClass(0, None, self.getP2IS().maxNumberOfLevels)
    ...
    cdef inline CSkipNode *getHead(PSkipList self): 
        return <CSkipNode *>(self.ptype.storage.baseAddress + self.getP2IS().o2Head)
    ...
    def range(PSkipList self, object fro=None, object to=None):
        ...
        if head.numberOfLevels > 0:
            if fro is None:
                ...
                free(cutList)
            while True:
                if <void*>cSkipNode == self.ptype.storage.baseAddress: break
                skipNode = entryClass.createProxyFA( cSkipNode ) 
                if to is not None and skipNode >= to:  break
    ...
    cdef CSkipNode ** getCutList(PSkipList self, object value) except NULL:
        """ Find the pointer at each level that will point to the Node after the insert
        """
        cdef:
            ...
            PSkipNode skipNode
            CSkipNode **cutList
        cutList = <CSkipNode **>malloc(nodeAtLevel.numberOfLevels*sizeof(CSkipNode *))
        try:
            while level >= 0:
                while True:
                    nextNodeAtLevel = getNextAtLevel(self.ptype.storage, nodeAtLevel, level)
                    if nextNodeAtLevel == self.ptype.storage.baseAddress: break 
                    skipNode = entryClass.createProxyFA( nextNodeAtLevel ) 
                    if skipNode >= value: break  
                    nodeAtLevel = nextNodeAtLevel
                cutList[level] = nodeAtLevel
                level -= 1
            return cutList
        except:
            if cutList: free(cutList)
            raise
    ...
    cdef CSkipNode * find(self, object key) except NULL:
        ...
        if head.numberOfLevels > 0:
            ...
            if cSkipNode != self.ptype.storage.baseAddress:
                skipNode = entryClass.createProxyFA( cSkipNode ) 
                if skipNode == key:  return cSkipNode
        raise KeyError(key)
    ...
    def insert(self, object value):
        ...
        if head.numberOfLevels < level:
            ...
            for i in range(head.numberOfLevels       ): newNext[i] = next[i]
            for i in range(head.numberOfLevels, level): newNext[i] = 0
            head.numberOfLevels = level
            # print 'head', CSkipNode2str(head, self.ptype.storage)
            
        cdef: 
            PSkipNode node = entryClass(value, level)
            CSkipNode **cutList = self.getCutList(node.getKey())
        # print 'inserting', CSkipNode2str(node.getP2IS(), self.ptype.storage)
        for i in range(level):
            setNextAtLevel(self.ptype.storage, node.getP2IS(), i, getNextAtLevel(self.ptype.storage, cutList[i], i) )
    ...
    cdef randomLevel(self):
        cdef int level = 1
        while rand() < limit4randomNumber: level += 1
        return level 
...
def defineTypes(Storage p): pass
...
cdef class Range(BindingRule):
    ...
    def __init__(self, str indexName, BindingRule fro, BindingRule to, bint shortCut=False):
        BindingRule.__init__(self, fro, to, shortCut=shortCut)
        self.indexName = indexName
        self.fromBindingRule = fro ; self.toBindingRule = to

    cdef getAll(Range self, query, QueryContext result):
        skipList = getattr(query.storage.root, self.indexName)
        for value in skipList.range(result.dick[self.fromBindingRule.name], result.dick[self.toBindingRule.name]):
