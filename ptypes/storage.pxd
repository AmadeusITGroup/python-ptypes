...
cdef class AssignedByValue(Persistent): pass
        
# p2InternalStructure points at a stand-alone object on the heap
cdef class AssignedByReference(Persistent): pass
...
cdef class PersistentMeta(type):
    ...
    cdef:
        ...
        int             assignmentSize # used in assignments, must equal to allocationSize when the assignment semantics of the type is store-by-value 
        Offset          (*resolve)(PersistentMeta ptype, Offset offset) except -1  

        type            proxyClass
        readonly str    __name__
    
    cdef inline void*  offset2Address(PersistentMeta ptype, Offset offset) except NULL:
        ptype.storage.assertNotClosed() 
        return ptype.storage.baseAddress + offset
    
    cdef inline Offset address2Offset(PersistentMeta ptype, void* address): return address - ptype.storage.baseAddress
    cdef inline Persistent createProxyFA(PersistentMeta ptype, void* address): return ptype.createProxy( ptype.address2Offset(address) )
    ...
    # an "embedded" area (e.g. fields). Never invoke these methods on pointers/offsets at 
    # areas containing a by-ref object! 
    # They create a proxy out of ptype
    cdef inline Persistent resolveAndCreateProxy(PersistentMeta ptype, Offset offset):
        return ptype.createProxy(ptype.resolve(ptype, offset) )
    cdef inline Persistent resolveAndCreateProxyFA(PersistentMeta ptype, void* address):
        return ptype.createProxy(ptype.resolve(ptype, address-ptype.storage.baseAddress))        
...
cdef class UInt(TypeDescriptor): pass
cdef class Float(TypeDescriptor): pass
cdef class __String(TypeDescriptor): pass
cdef class Set(TypeDescriptor): pass
cdef class Dict(TypeDescriptor): pass
cdef class DefaultDict(Dict): pass
cdef class List(TypeDescriptor):pass
...
cdef class PHashEntry(AssignedByValue):
    pass
#     cdef inline CHashEntry* getP2IS(self): return <CHashEntry *>self.p2InternalStructure
#     cdef inline void* getP2Key  (PHashEntry self): return self.p2InternalStructure + (<HashEntryMeta>self.ptype).o2Key     
#     cdef inline void* getP2Value(PHashEntry self): return self.p2InternalStructure + (<HashEntryMeta>self.ptype).o2Value
# 
#     cdef inline Persistent getKey(self):
#         return (<HashEntryMeta>self.ptype).keyClass  .resolveAndCreateProxyFA(self.getP2Key  ())
#     cdef inline setKey(self, key):
#         (<HashEntryMeta>self.ptype).keyClass.assign(self.getP2Key(), key)
#         
#     cdef inline Persistent getValue(self):
#         return (<HashEntryMeta>self.ptype).valueClass.resolveAndCreateProxyFA(self.getP2Value())
#     cdef inline setValue(self, value):
#         (<HashEntryMeta>self.ptype).valueClass.assign(self.getP2Value(), value)
...
cdef class PHashTable(AssignedByReference):
    ...
    cdef inline Offset getO2Entry(self, unsigned long i): 
        return self.getP2IS().o2EntryTable + i*self.hashEntryClass.assignmentSize
    
    cdef inline CHashEntry* getP2Entry(self, unsigned long i):
        return <CHashEntry*><unsigned long>(self.ptype.storage.baseAddress + self.getO2Entry(i))

    cdef inline Offset getO2Key  (self, Offset      o2Entry): return o2Entry + self.o2Key     
    cdef inline Offset getO2Value(self, Offset      o2Entry): return o2Entry + self.o2Value
    cdef inline void*  getP2Key  (self, CHashEntry* p2Entry): return (<void*>p2Entry) + self.o2Key     
    cdef inline void*  getP2Value(self, CHashEntry* p2Entry): return (<void*>p2Entry) + self.o2Value
    ...
    cdef inline Persistent getValue(self, CHashEntry* p2Entry):
        return self.valueClass.resolveAndCreateProxyFA(self.getP2Value(p2Entry))
    cdef inline setValue(self, CHashEntry* p2Entry, value):
        self.valueClass.assign(self.getP2Value(p2Entry), value)
    
cdef class PDefaultHashTable(PHashTable): pass

cdef class PString(AssignedByReference):
    cdef inline char *getCharPtr(self): return <char*>self.p2InternalStructure+sizeof(int)
    cdef inline int   getSize   (self): return (<int*>self.p2InternalStructure)[0]
    cdef inline bytes getString (self): return self.getCharPtr()[:self.getSize()]
...
cdef class PList(AssignedByReference):
    ...
    cdef inline CList *getP2IS(self): return <CList *>self.p2InternalStructure
...
#     cdef inline int equals(PField self, Structure owner, other) except -1:
#         cdef Offset offset2 = self.ptype.resolve(self.ptype, owner.offset + self.offset)
#         return self.ptype.equals(offset2, other) 
...
cdef class MemoryMappedFile(object):    
    ...
    cdef inline assertNotClosed(self):
        if self.baseAddress == NULL:
            raise ValueError('Storage {self.fileName} is closed.'.format(self=self))
