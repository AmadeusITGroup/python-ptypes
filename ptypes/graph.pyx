...
cdef class PNode(AssignedByReference): 
    ...
    cdef inline CNode *getP2IS   (PNode self): return <CNode *>self.p2InternalStructure 
    cdef inline void  *getP2Value(PNode self): return self.p2InternalStructure + (<NodeMeta>self.ptype).o2Value 

    cdef inline Persistent getValue(self): 
        return (<NodeMeta>self.ptype).valueClass.resolveAndCreateProxyFA(self.getP2Value())
...
#         assert value is None or value.ptype.storage is self.ptype.storage, (value.ptype.storage, self.ptype.storage) 
        self.getP2IS().o2FirstInEdgeKind = 0
        self.getP2IS().o2FirstOutEdgeKind = 0
        self.setValue(value)
    
    def __repr__(self):
        return "<persistent graph node '{0}' @offset {1} referring to {2} >".format(self.ptype.__name__, hex(self.offset), self.getValue())
...
#         print "getP2EdgeKind", self, hex(o2Name), edgeDirection, createNew
        ...
        if edgeDirection == In:
            ...
            raise ValueError("In or Out must be set in the 'edgeDirection' parameter!")
        ...
        while o2EdgeKind:
            p2CEdgeKind = <CEdgeKind*>(self.ptype.storage.baseAddress + o2EdgeKind) 
            if p2CEdgeKind.o2ClassName == edgeClass.o2Name: 
#                 print 'found o2EdgeKind', hex(o2EdgeKind), createNew, (<unsigned long> p2CEdgeKind)          
        ...
        if o2EdgeKind == 0: 
            if createNew:
                ...
                p2MatchingCEdgeKind = <CEdgeKind*>(self.ptype.storage.baseAddress + o2EdgeKind)
                ...
                raise ValueError("No edge of type '{0}'.".format(edgeClass.__name__))          
    ...
    def edges(PNode self, EdgeMeta edgeClass, EdgeDirection edgeDirection):
        assert edgeClass.storage is self.ptype.storage, (edgeClass.storage, self.ptype.storage) 
        cdef:
            CEdgeKind       *p2CEdgeKind
            Offset   o2Edge      = 0
            CEdge           *p2edge
        try: p2CEdgeKind = self.getP2EdgeKind(edgeClass, edgeDirection)
        except ValueError: pass
        else: o2Edge = p2CEdgeKind.o2FirstEdge
        while o2Edge:
...
cdef class EdgeMeta(PersistentMeta):
    cdef:
        ...
        Offset   o2Name # cache it; avoid calling stringRegistry.get(ptype.__name__)
        Offset          o2Value
...
cdef class PEdge(AssignedByReference):      
    ...
    cdef inline CEdge *getP2IS   (PEdge self): return <CEdge *>self.p2InternalStructure 
    cdef inline void  *getP2Value(PEdge self): return self.p2InternalStructure + (<EdgeMeta>self.ptype).o2Value 

    cdef inline Persistent getValue(self): 
        return (<EdgeMeta>self.ptype).valueClass.resolveAndCreateProxyFA(self.getP2Value())
    cdef inline setValue(self, value): 
        (<EdgeMeta>self.ptype).valueClass.assign(self.getP2Value(), value)

    def __init__(PEdge self, PNode fromPNode not None, PNode toPNode not None, Persistent value = None):
        cdef EdgeMeta edgeClass = <EdgeMeta>self.ptype
#         print '_init_', self.offset, edgeClass.o2Name
        assert fromPNode.ptype.storage is edgeClass.storage, (fromPNode.ptype.storage, edgeClass.storage)
        assert   toPNode.ptype.storage is edgeClass.storage, (toPNode.ptype.storage, edgeClass.storage)
#         assert value is None or value.ptype.storage is edgeClass.storage, (value.ptype.storage, edgeClass.storage)
        if fromPNode.ptype != edgeClass.fromNodeClass: raise ValueError("{0} expects an instance of {expected} as fromNode, not a {fromPNode.ptype} instance".format(edgeClass.__name__, expected=edgeClass.fromNodeClass, fromPNode=fromPNode))
        if toPNode  .ptype != edgeClass.toNodeClass  : raise ValueError("{0} expects an instance of {expected} as toNode, not a {toPNode.ptype} instance"  .format(edgeClass.__name__, expected=edgeClass.toNodeClass  , toPNode=toPNode))
        self.getP2IS().o2FromNode = fromPNode.offset
        self.getP2IS().o2ToNode   = toPNode  .offset 

        cdef:        
            # find the edge types of our class
            CEdgeKind *p2CEdgeKindOfFromNode = fromPNode.getP2EdgeKind(edgeClass, Out, 1)
            CEdgeKind *p2CEdgeKindOfToNode   = toPNode  .getP2EdgeKind(edgeClass, In , 1) 
        ...
        p2CEdgeKindOfFromNode.o2FirstEdge = p2CEdgeKindOfToNode.o2FirstEdge = self.offset
        self.setValue(value)
    ...
    property fromNode:
        def __get__(PEdge self): return self.getFromNode()

    property toNode:
        def __get__(PEdge self): return self.getToNode()
    ...
    def __repr__(self):
        return "<persistent graph edge '{0}' @offset {1} referring to {2} >".format(self.ptype.__name__, hex(self.offset), self.getValue())
...
cdef class NodeContents(BindingRule):
    ...
    cdef getAll(self, query, QueryContext result):
        ...
        try:
            ...
            e.args = e.args[0] + " while accessing the contents of {0} in bindingRule '{1}'".format( node, self.name),
            raise
...
cdef class NodeAttribute(BindingRule):
    ...
    def __init__(self, BindingRule nodeBindingRule, str attributeName=None, bint shortCut=False):
        BindingRule.__init__(self, nodeBindingRule, shortCut=shortCut)
        self.nodeBindingRule = nodeBindingRule 
    ...
    cdef getAll(self, query, QueryContext result):
        cdef PNode node = result.getattr(self.nodeBindingRule.name)
        try:
            attributeValue = getattr(node.getValue(), self.attributeName or self.name)
            result.setattr(self.name, attributeValue)
        except Exception as e:
            e.args = e.args[0] + " while accessing attribute '{0}' of the contents of {1} in bindingRule '{2}'".format(self.attributeName, node, self.name),
            raise
        self.getAllRecursively(query, result)
...
cdef class FindEdge(BindingRule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get('fromNode', None) is None and kwargs.get('toNode', None) is None:
                raise NotImplementedError('At least one of fromNode or toNode has tobe specified.')
        self.__init(**kwargs)
    
    def __init(self, str edgeKind, BindingRule fromNode=None, BindingRule toNode=None, bint shortCut=False):
        BindingRule.__init__(self, fromNode, toNode, shortCut=shortCut)
        self.fromNodeBindingRule, self.edgeKind, self.toNodeBindingRule  = fromNode, edgeKind, toNode
        if self.fromNodeBindingRule is not None:
            ...
            assert False, "both self.toNodeBindingRule and self.fromNodeBindingRule are None!"
...
#         return node.edges(edgeClass, In)
    cdef getAll(self, query, QueryContext result):
        ...
        if self.fromNodeBindingRule is not None:
            ...
            otherNode = result.dick[self.toNodeBindingRule.name] if self.toNodeBindingRule else None
        elif self.toNodeBindingRule is not None:
