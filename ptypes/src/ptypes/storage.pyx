...
class DbFullException(Exception): pass
class RedoFullException(Exception): pass

cdef class Persistent(object):
    """ Base class for all the proxy classes for persistent objects. 
    """
    def __repr__(self):
        return "<persistent {0} object @offset {1}>".format(self.ptype.__name__, hex(self.offset))
    
    def __hash__(self, ): return self.offset
     
    def __richcmp__(Persistent self, other, int op): return bool(self.richcmp(other, op))
    
    property id:
        """ Read-only property returning a tuple that uniquely identifies the object. 
        """
        def __get__(self):
    ...
    cdef store(Persistent self, void *target): 
        """ Store self to target by ref or by value as the type of self dictates
        """    
        raise NotImplementedError()        
    cdef revive(Persistent p): pass 
...
cdef class AssignedByValue(Persistent):
    # p2InternalStructure points at a persistent object embedded inside another persistent object
    ...
    # works only with persistent values. Can be specialized in derived classes for specific types.
    # In the overrider call this if other is persistent, otherwise use the special code   
    cdef int richcmp(AssignedByValue self, other, int op) except? -123:
        ...
        if self.ptype == pother.ptype: 
            doesDiffer = memcmp(self.p2InternalStructure, pother.p2InternalStructure, self.ptype.assignmentSize)
        else:
            doesDiffer = 1
        if op==2: return doesDiffer == 0
        if op==3: return doesDiffer != 0
...
cdef class AssignedByReference(Persistent):
    ...
    cdef store(AssignedByReference self, void *target): 
        ...
        (<Offset*>target)[0] = self.p2InternalStructure - self.storage.baseAddress
...
cdef class PersistentMeta(type):
    ...
    """
    @classmethod
    def typedef(PersistentMeta meta, Storage storage, str className, type proxyClass, *args ):
        """ Create an instance of this meta-class (or of a meta-class derived from this one) and call its __init__ method  
             
            @return: the initialised instance (representing a newly created persistent type)
        """
        ...
        LOG.debug('Created {ptype} from meta-class {meta} using proxy {proxyClass} with arguments {args}'
                  .format(ptype=ptype, meta=meta, proxyClass=proxyClass, args=args)
                  )
        return ptype
     
    # This method is called to initialise an instance of this meta-class when a new  
    # persistent type has just been created  
    def __init__(PersistentMeta ptype, Storage storage, str className, type proxyClass, int allocationSize):
        ...
        if not ptype.__name__.startswith('__'): # isinstance(ptype, HashTableEntry)
    ...
    def __call__(PersistentMeta ptype, *args, **kwargs):
        ...
        """
        if ptype.isAssignedByValue():
            raise TypeError('{ptype} exhibits store-by-value semantics and therefore can only be instantiated inside a container (e.g. in Structure)'.format(ptype=ptype))
        cdef Persistent self = ptype.createProxy(allocateStorage(ptype)) # always by ref
    ...
    cdef Persistent createProxy(PersistentMeta ptype, Offset offset):
        ...
        if offset: 
            if ptype.storage.realFileSize < offset:
                print("Corruption: offset {offset} is outside the mapped memory! - Aborting.".format(offset=offset))
                abort()
            self = ptype.__new__(ptype) # ptype.proxyClass.__new__(ptype.proxyClass) #   
            self.p2InternalStructure =  ptype.offset2Address(offset)
            self.ptype = ptype
            self.storage = ptype.storage
            self.offset = offset
#             LOG.debug('createProxy: {0} {1} ==> {2}'.format(ptype.proxyClass, offset, repr(self)))
            self.revive()
            return self
        else: return None  
    ...
    cdef assign(PersistentMeta ptype, void *target, source, ):
        """ Assign source to target converting source to an instance of ptype as needed
        """    
    ...
    cdef int isAssignedByValue(PersistentMeta ptype) except? -123: 
        ...
        if isByValue:
            ...
            raiseTypeError = not issubclass(ptype.proxyClass, AssignedByReference)
        if raiseTypeError:
            raise TypeError("The proxyClass {0} must be a subclass of  either 'AssignedByValue' or 'AssignedByReference'."
    ...
    cdef assertType(PersistentMeta ptype, Persistent persistent):
        if persistent:
            if persistent.ptype.storage is not ptype.storage:
                raise ValueError("Expected a persistent object in {0}, not in {1}!"
                                 .format(ptype.storage, persistent.ptype.storage)
                                 )
            if not issubclass(persistent.ptype, ptype):
                raise TypeError("Expected {0}, found {1}".format(ptype, persistent.ptype))
    
    def __repr__(ptype): return "<persistent class '{0}'>".format (ptype.__name__) #ptype.__class__.__name__
...
cdef class TypeDescriptor(object):
    ...
    def __init__(TypeDescriptor self, str className=None):
        if className is None: className = self.__class__.__name__
    ...
    def __getitem__(TypeDescriptor self, typeParameters):
        ...
        if self.typeParameters:
            raise ValueError("The parameters of type {self.className} are already set to {self.typeParameters}".format(self=self))
    ...
    def verifyTypeParameters(self, tuple typeParameters):
        if self.minNumberOfParameters is None and typeParameters: 
            raise TypeError("The type {self.className} does not accept parameters!".format(self=self))
    
        if self.minNumberOfParameters and len(typeParameters) < self.minNumberOfParameters: 
            raise TypeError("Type {self.className} must have at least {self.minNumberOfParameters} parameter(s), "
                             "found {typeParameters}".format(self=self, typeParameters=typeParameters)
                             )
        if self.maxNumberOfParameters and len(typeParameters) > self.maxNumberOfParameters: 
            raise TypeError("Type {self.className} must have at most {self.maxNumberOfParameters} parameter(s), "
                             "found {typeParameters}".format(self=self, typeParameters=typeParameters)
                             )
...
cdef class UIntMeta(PersistentMeta):
    ...
    def __init__(UIntMeta ptype, 
                 Storage storage, 
                 str className,
                 type proxyClass, # =None
                 ):
        assert issubclass(proxyClass, PUInt), proxyClass
        PersistentMeta.__init__(ptype, storage, className, proxyClass, sizeof(unsigned long))
...
cdef class PUInt(AssignedByValue): 
    ...
    def __repr__(self):
        return "<persistent {0} object '{1}' @offset {2}>".format(self.ptype.__name__, self.getP2IS()[0], hex(self.offset))
    ...
    def __hash__(self, ): return hash(self.getP2IS()[0])
    ...
    cdef int richcmp(PUInt self, other, int op) except? -123: 
        ...
        if isinstance(other, PUInt):
            ...
            if isinstance(other, int):
                ...
                if op==2: return False # self == other 
                if op==3: return True  # self != other
                raise TypeError('{0} does not define a sort order for {1}!'.format(self.ptype, other))
        if op==0: return self.getP2IS()[0] <  otherValue #self  < other
        if op==1: return self.getP2IS()[0] <= otherValue #self <= other
        if op==2: return self.getP2IS()[0] == otherValue # self == other 
        if op==3: return self.getP2IS()[0] != otherValue # self != other
        if op==4: return self.getP2IS()[0] >  otherValue # self  > other
        if op==5: return self.getP2IS()[0] >= otherValue # self >= other
...
cdef class FloatMeta(PersistentMeta):
    ...
    def __init__(self, 
        ...
        if proxyClass is None: proxyClass = PFloat
        assert issubclass(proxyClass, PFloat), proxyClass
        PersistentMeta.__init__(self, storage, className, proxyClass, sizeof(double))
...
cdef class StringMeta(PersistentMeta):
    ...
    def __call__(StringMeta ptype, object volatileString):
        ...
        cdef:
            ...
            PString self = ptype.createProxy(ptype.storage.allocate(sizeof(int) + size)) 
    ...
    def __init__(StringMeta ptype, 
        ...
        if proxyClass is None: proxyClass = PString
...
cdef class PString(AssignedByReference): 
    ...
    def __repr__(self):
        return "<persistent {0} object {1} @offset {2}>".format(self.ptype.__name__, repr(self.getString()[:self.getSize()]), hex(self.offset))
    ...
    # The offset is not OK here: it must match that of the volatile object!
    def __hash__(self, ): return hash(self.getString())
    ...
    cdef int richcmp(PString self, other, int op) except? -123: 
        ...
        if isinstance(other, PString):
            ...
            if isinstance(other, str):
                ...
                if op==3: return True # self != other
                raise TypeError('{0} does not define a sort order for {1}!'.format(self.ptype, other))
        doesDiffer = memcmp(selfValue,  otherValue, min(self.getSize(), otherSize))
#         LOG.warn( repr(( 'comparing', repr(selfValue), self.getSize(), repr(otherValue), otherSize, doesDiffer)))
        if not doesDiffer: doesDiffer =  self.getSize() - otherSize
        if op==0: return doesDiffer <  0 #self  < other
        if op==1: return doesDiffer <= 0 #self <= other
        if op==2: return doesDiffer == 0 # self == other 
        if op==3: return doesDiffer != 0 # self != other
        if op==4: return doesDiffer >  0 # self  > other
        if op==5: return doesDiffer >= 0 # self >= other
        assert False, "Unknown operation code '{0}".format(op)
...
cdef class HashEntryMeta(PersistentMeta):
    def __init__(self, 
        ...
        PersistentMeta.__init__(self, storage, className, proxyClass,
                                self.o2Value + (valueClass.assignmentSize if valueClass else 0) 
                                )
    ...
    def reduce(self): 
        assert False, "The name of HashEntryMeta instances must start with '__' to prevent pickling them!"


cdef class PHashEntry(AssignedByValue): pass
...
cdef class HashTableMeta(PersistentMeta):
    ...
    @classmethod
    def typedef(PersistentMeta meta, Storage storage, str className, type proxyClass, PersistentMeta keyClass, PersistentMeta valueClass=None):
        if keyClass is None:
            raise TypeError("The type parameter specifying the type of keys cannot be {0}"
                             .format(keyClass))
        cdef:
            str entryName =  ( (  '__{keyClass.__name__}And{valueClass.__name__}AsHashEntry'
            ...
            PersistentMeta entryClass = HashEntryMeta.typedef(storage, entryName, PHashEntry, keyClass, valueClass)
            
        return super(HashTableMeta, meta).typedef(storage, className, proxyClass, entryClass)    
    ...
    def __init__(self, 
        ...
        PersistentMeta.__init__(self, storage, className, proxyClass, sizeof(CHashTable))
        self.hashEntryClass =  hashEntryClass
        
    def reduce(self):
        return ('typedef', self.__name__, self.__class__, self.proxyClass, 
                ('PersistentMeta', None if self.hashEntryClass.keyClass is None else self.hashEntryClass.keyClass.__name__),
                ('PersistentMeta', None if self.hashEntryClass.valueClass is None else self.hashEntryClass.valueClass.__name__),
                )
...
cdef class PHashTable(AssignedByReference):
    ...
    def __init__(PHashTable self, unsigned long size, ):
        assert size > 0, 'The size of a HashTable cannot be {size}.'.format(size=size)
        actualSize = size*3/2  
        ...
        cdef unsigned long hashTableSize = actualSize * self.hashEntryClass.assignmentSize
        self.getP2IS().o2EntryTable = self.storage.allocate(hashTableSize)
        memset(self.storage.baseAddress + self.getP2IS().o2EntryTable, 0, hashTableSize)
        LOG.debug("Created new HashTable  {4} of type '{0}', requested_size={1} actual size={2} allowed capacity={3}."
                 .format(self.ptype.__name__, size, actualSize, self.getP2IS()._capacity, self))
    ...
    def __getitem__(self, key):
        """ Get they value associated with key
            ...
            If the value class of the hash table is None (i.e. applying the operation on a set),
            then the persistent version of the passed in key is returned.
    ...
    cdef missing(self, CHashEntry* p2Entry, key): raise KeyError(key)
        
    def __setitem__(self, key, value):
        """ Set they value associated with key
        
            If the value class of the hash table is None (i.e. applying the operation on a set),
            then value is silently ignored.
        """
        cdef CHashEntry* p2Entry = self._findEntry(key)
    ...
    cpdef Persistent get(self, object key, value=None):
        """ Return the matching persistent version of a volatile key.
            ...
            If the hash table does not have a matching persistent key, then the volatile key is 
            persisted by invoking the constructor of the key class with the volatile key as its sole 
            argument. The persistent key constructed this way is added to the hash table.
            In order to be able to use this method, the constructor of the key class must be able
            to accept the volatile key as its sole argument.
            If the key is persisted in the current invocation, then the optional value is
            also associated to the key, provided the value class of the hash table is not None 
            (i.e. applying the operation on a dictionary). If the value class is None, then
            the value is always silently ignored. If the hash table already contained a matching
            persistent key before the invocation, then the value associated with the key is not
            altered.
    ...
    def itervalues(self):
        ...
        if self.hashEntryClass.valueClass:
            ...
            raise TypeError('Cannot iterate over the values: no value class is defined. (Is this not a Set?)') 
    def iteritems(self):
        cdef:
            unsigned long i
            CHashEntry* p2Entry
        if self.hashEntryClass.valueClass:
            ...
            raise TypeError('Cannot iterate over the items: no value class is defined. (Is this not a Set?)') 
                
    cdef incrementUsed(self):   
        if self.getP2IS()._used >= self.getP2IS()._capacity:
            raise DbFullException("HashTable of type '{0}' is full, current capacity is {1}.".format(self.ptype.__name__, self.getP2IS()._capacity))
...
cdef class ListMeta(PersistentMeta):
    ...
    @classmethod
    def typedef(PersistentMeta meta, Storage storage, str className, type proxyClass, PersistentMeta valueClass):
        if valueClass is None:
            raise TypeError("The type parameter specifying the type of list elements cannot be None.")
        return super(ListMeta, meta).typedef(storage, className, proxyClass, valueClass)    
    ...
    def __init__(self, 
        ...
        PersistentMeta.__init__(self, storage, className, proxyClass, sizeof(CList))
        self.valueClass  = valueClass
...
cdef class PList(AssignedByReference):
    ...
    cdef CListEntry *newEntry(self, value, Offset* o2NewEntry):
        ...
        o2NewEntry[0] = self.storage.allocate(sizeof(CListEntry)  + valueClass.assignmentSize)
        cdef CListEntry *p2NewEntry = <CListEntry *>(self.storage.baseAddress + o2NewEntry[0]) 
        valueClass.assign((<void*>p2NewEntry) + (<ListMeta>(self.ptype)).o2Value, value)
        return p2NewEntry
    ...
    cpdef append(PList self, object value):
        ...
        if self.getP2IS().o2LastEntry == 0: 
            ...
            # Caveat! http://stackoverflow.com/questions/11498441/what-is-this-kind-of-assignment-in-python-called-a-b-true
            p2LastEntry =  <CListEntry *>(self.storage.baseAddress + self.getP2IS().o2LastEntry)
    ...
    def __iter__(self):
        ...
        while o2Entry:
            ...
            # yield (<ListMeta>(self.ptype)).valueClass.resolveAndCreateProxy(& p2Entry.o2Key) XXX o2Value lehet mas is
...
cdef class StructureMeta(PersistentMeta):
    ...
    def __init__(ptype, className, bases, dict attribute_dict):
        ...
        if storage == None:
            raise Exception("Types with {ptype.__class__.__name__} as __metaclass__ must be defined "
                            "in the populateSchema() method of Storage subclasses!".format(ptype=ptype))
        ...
        LOG.debug('Created {ptype} from meta-class {meta} using proxy {proxyClass} allocationSize {allocationSize}'
                  .format(ptype=ptype, meta=type(ptype), proxyClass=Structure, allocationSize=ptype.allocationSize)
                  )
    ...
    cdef addField(StructureMeta ptype, name, PersistentMeta fieldType):
        ...
        LOG.debug('Added {field} to {ptype}' .format(field=pfield, ptype=ptype) )
        
    def reduce(ptype):
        d = dict(ptype.__dict__)
        for name in ['__metaclass__', '__dict__', '__weakref__', '__module__', 'storage']:
            d.pop(name, None)
        for k, v in d.items():
#             if isinstance(v, PersistentMeta):
            if isinstance(v, PField):
                del d[k]
        return 'StructureMeta', ptype.__name__, ptype.__bases__, d, ptype.fields
            

cdef class Structure(AssignedByReference):  
    """ A structure is like a mutable named tuple.
    
        Structures are usable as hash keys (they are hashable), but prepare for surprises
        if you do so and change the contents of the structure after initialisation.
        
        Structures lack the 'greater than / less than' relational operators, 
        so they are not usable as keys in skip lists.
        
        Structures can be compared for (non-)equality. They are compared field-by-field, accessing via ``getattr()``.
        Extra fields on the compared-to-object are ignored. 
        
        Accessing the ``contents`` attribute of a structure instance will return a named tuple.
        Assigning to the attribute will set the contents of the structure to the assigned value, 
    ...
    # The offset is not OK here: it must match that of the volatile object!
    def __hash__(self, ): return hash(self.get())
    ...
    cdef int richcmp(Structure self, other, int op) except? -123:
        ...
        if other is None:
            ...
            doesDiffer = False
#             for attribName in dir(other):
#                 if attribName.startswith('_'): continue
#                 if attribName not in (<StructureMeta>self.ptype).pfields.keys():
#                     doesDiffer = True
#                     break
            if not doesDiffer:
                for pfield in (<StructureMeta>self.ptype).pfields.values():
                    value = pfield.get(self)
                    try: otherValue = getattr(other, pfield.name)
                        ...
                        if value is None:
                            ...
                            if value.richcmp(otherValue, 3):
                                doesDiffer = True
                                break 
        if op==2: return not doesDiffer
        if op==3: return doesDiffer 
        raise TypeError('{0} does not define a sort order!'.format(self.ptype))
    ...
    cdef get(Structure self):    
        cdef:
            ...
            list    values = list()
            type    NamedTupleClass = (<StructureMeta>self.ptype).NamedTupleClass    
    ...
    property contents:
        def __get__(Structure self)         : return self.get()    
        def __set__(Structure self, value)  : self.set(value)   
    
cdef class PField(object):
        
    def __init__(PField self, int offset, str name, PersistentMeta ptype=None): 
        self.offset = offset # offset inside the structure
        self.ptype = ptype
        self.name = name
    
    def __repr__(self):
        return 'PField({1}, offset={0}, ptype={2})'.format(self.offset, self.name, self.ptype)
    
    property size:
        def __get__(PField self): return self.ptype.assignmentSize   
        
    def __get__(PField self, Structure owner, ownerClass):
        if owner is None: return self
        else:             return self.get(owner)
    
    cdef Persistent get(PField self, Structure owner):
        assert owner is not None
        assert owner.storage is self.ptype.storage, (owner.storage, self.ptype.storage)
...
cdef class MemoryMappedFile(object):    
    ...
    def __init__(self, str fileName, int numPages=0):
        ...
        try: self.fd = os.open(self.fileName,  os.O_RDWR) 
        except OSError:
            LOG.debug("Creating new file '{self.fileName}'".format(self=self)) 
            assert numPages > 0, 'The database cannot have {numPages} pages.'.format(numPages=self.numPages)
            self.isNew = 1 
            ...
            LOG.debug("Opened existing file '{self.fileName}'".format(self=self))
            self.isNew = 0 
            self.realFileSize = os.fstat(self.fd).st_size  
            self.numPages = int(self.realFileSize / PAGESIZE) 
        cdef int myerr=0
        self.baseAddress = mmap(NULL, self.realFileSize, PROT_READ|PROT_WRITE, MAP_SHARED, self.fd, 0)
        if self.baseAddress == MAP_FAILED:
            myerr = errno # save it, any c-lib call may overwrite it!
            raise Exception('Could not map {self.fileName}: {0}'.format(strerror(myerr), self=self))
        self.endAddress = self.baseAddress + self.realFileSize
        LOG.debug('Mmapped {self.fileName} memory region {0}-{1}'.format(hex(<unsigned long>self.baseAddress), hex(self.realFileSize), self=self))
        if self.isNew:
            self._initialize()
        else:
            self._mount()
    ...
    cpdef flush(self, bint async=0):
        cdef int myerr=0
        LOG.debug('Msyncing {self.fileName} memory region {0}-{1}'.format(hex(<unsigned long>self.baseAddress), hex(self.realFileSize), self=self))
        if msync(self.baseAddress, self.realFileSize, MS_ASYNC if async else MS_SYNC ):
            myerr = errno # save it, any c-lib call may overwrite it!
            raise Exception('Could not sync {self.fileName}: {0}'.format(strerror(myerr), self=self))
    
    cpdef close(self):
        self.assertNotClosed()
        cdef int myerr=0
        LOG.debug('Unmapping {self.fileName} memory region {0}-{1}'.format(hex(<unsigned long>self.baseAddress), hex(self.realFileSize), self=self))
        if munmap(self.baseAddress, self.realFileSize):
            myerr = errno
            raise Exception('Could not unmap {self.fileName}: {0}'.format(strerror(myerr), self=self))            
        os.close(self.fd)
        self.baseAddress = NULL

    def __repr__(self):
        return "<{self.__class__.__name__} '{self.fileName}'>".format(self=self)
    
#    
cdef struct CRedoFileHeader:
    char magic[lengthOfMagic] 
    # offsets to the first trx header and to where the next trx header probably can be written 
    # (just a hint, a shortcut to a trx header near to the tail, need to verify if it is really unused, 
...
cdef class Redo(MemoryMappedFile):
    ...
    def __init__(self, str fileName, int numPages=0):
        ...
        self.p2Tail = <CTrxHeader*>(self.baseAddress + self.p2FileHeader.o2Tail)
        while self.p2Tail.length != 0 and self.p2Tail.length < self.realFileSize:
            MD5_Init(&md5Context)
            ...
            if memcmp(self.p2Tail.checksum, checksum, sizeof(MD5_checksum)):
                break
            self.p2Tail = <CTrxHeader *>(<void*>self.p2Tail + sizeof(CTrxHeader) + self.p2Tail.length)
    ...
    def _initialize(self):       
        ...
        self.p2FileHeader.o2Tail = self.p2FileHeader.o2firstTrx = sizeof(CRedoFileHeader) #numMetadata*PAGESIZE
 
    def _mount(self):
        LOG.info("Mounting existing journal '{self.fileName}'".format(self=self))
        self.p2FileHeader = <CRedoFileHeader*>self.baseAddress
        if any( self.p2FileHeader.magic[j] != ptypesRedoMagic[j] for j in range(len(ptypesRedoMagic)) ):
            raise Exception('File {self.fileName} is incompatible with this version of ptypes!'.format(self=self))
...
cdef struct CTrxHeader:
    # A transaction starts with a trx header, which is followed by a set of redo records with the given total length.
    # It is committed if the checksum is correct (it is filled in after all the redo records).
    # We rely on the "sparse file" mechanism of the kernel to initialize this data structure to zeros.
    unsigned long length
    MD5_checksum checksum

cdef struct CRedoRecordHeader:
    # A redo record consists of a redo record header followed by a body
    # The body has the given length and is opaque; it is copied byte-by-byte to the target offset when the redo is applied.      
    Offset offset
    unsigned long length
...
#         self.p2FileHeader = redo.p2FileHeader
        ...
        self.p2CRedoRecordHeader = <CRedoRecordHeader*>(<void*>redo.p2Tail + sizeof(CRedoRecordHeader))
        MD5_Init(&self.md5Context)
        
    cdef save(self, const void *sourceAddress, unsigned long length):
        assert self.redo
        assert self.storage
        if <void*>self.p2CRedoRecordHeader + sizeof(CRedoRecordHeader) + length >= self.redo.endAddress:
            raise RedoFullException("{0} is full.".format(self.redo.fileName))
        self.p2CRedoRecordHeader.offset = sourceAddress - self.storage.baseAddress
        self.p2CRedoRecordHeader.length = length
        # validate source range
        assert sourceAddress > self.storage.baseAddress
        assert self.p2CRedoRecordHeader.offset + length < self.storage.realFileSize
#         cdef Offset newRedoOffset
        ...
        MD5_Update(&self.md5Context, <void*>(self.p2CRedoRecordHeader), sizeof(CRedoRecordHeader) + length)
        self.p2CRedoRecordHeader = <CRedoRecordHeader*>(redoRecordPayload + length) 
        
    cdef commit(self, lazy=False):
        MD5_Final((self.redo.p2Tail.checksum), &self.md5Context, )
        self.redo.p2Tail.length = <void*>self.p2CRedoRecordHeader - <void*>self.redo.p2Tail
...
cdef class Storage(MemoryMappedFile):    
    ...
    cdef registerType(self, PersistentMeta ptype):
        if hasattr(self.schema, ptype.__name__):
            raise Exception("Redefinition of type '{ptype.__name__}'.".format(ptype=ptype)) 
    ...
    def _initialize(self):
        ...
        for i in range(numMetadata):
            self.p2FileHeader = self.p2FileHeaders[i] = <CDbFileHeader*>self.baseAddress + i
    ...
    def _mount(self):
        ...
        for i in range(numMetadata):
            self.p2FileHeader = self.p2FileHeaders[i] = <CDbFileHeader*>self.baseAddress + i
            if any( self.p2FileHeader.magic[j] != ptypesMagic[j] for j in range(len(ptypesMagic)) ):
                raise Exception('File {self.fileName} is incompatible with this version of the graph DB!'.format(self=self))
            if self.p2LoHeader == NULL or self.p2LoHeader.revision > self.p2FileHeader.revision: 
                self.p2LoHeader = self.p2FileHeader
            if self.p2HiHeader == NULL or self.p2HiHeader.revision < self.p2FileHeader.revision: 
                self.p2HiHeader = self.p2FileHeader
        if self.p2HiHeader.status != 'C': # roll back
            LOG.info("Latest shutdown was incomplete, restoring previous metadata.")
    ...
    def __init__(self, fileName, unsigned long fileSize=0, unsigned long stringRegistrySize=0):
        cdef unsigned long numPages = 0 if fileSize==0 else (fileSize-1)/PAGESIZE + 1 + numMetadata  
        MemoryMappedFile.__init__(self, fileName, numPages)
        LOG.debug("Highest metadata revision is {0}".format(self.p2HiHeader.revision)) 
        LOG.debug("Lowest metadata revision is {0}" .format(self.p2LoHeader.revision)) 
        LOG.debug("Using metadata revision {0}" .format(self.p2FileHeader.revision)) 
        if not self.isNew and fileSize > self.realFileSize: 
            raise Exception("File {self.fileName} is of size {self.realFileSize}, "
                            "cannot resize it to {fileSize}.".format(self=self, fileSize=fileSize))              
        ...
        cdef:
            PersistentMeta ListOfStrings = self.define( List('__ListOfStrings')[self.schema.String] ) 
            PersistentMeta SetOfStrings  = self.define( Set ('__SetOfStrings' )[self.schema.String] )
         
        if self.p2FileHeader.o2StringRegistry:
            LOG.debug("Using the existing stringRegistry")
            self.stringRegistry = SetOfStrings.createProxy(self.p2FileHeader.o2StringRegistry)
        else:            
        ...
        LOG.debug('self.p2FileHeader.o2StringRegistry: {0}'.format(hex(self.p2FileHeader.o2StringRegistry)))
        ...
        if self.createTypes:
            ...
            self.pickledTypeList = <PList>(ListOfStrings.createProxy(self.p2FileHeader.o2PickledTypeList))
            for s in self.pickledTypeList:
                ...
                if t[0] == 'typedef':
                    ...
                    typeParams = [
                        ...
                        if isinstance(typeParam, tuple) and typeParam[0] == 'PersistentMeta'                            
                        else typeParam
                    ...
                    for fieldName, fieldTypeName in t[4]:
                        attributeDict[fieldName] = getattr(self.schema, fieldTypeName)
                    try:
                        threadLocal.currentStorage = self
    ...
    cpdef close(self):
        """ Flush and close the storage.
            ...
            Triggers a garbage collection to break any unreachable cycles referencing the storage. 
        """
        LOG.debug("Closing {}".format(self))
        self.assertNotClosed()
        suspects = [o for o in gc.get_referrers(self) 
            if isinstance(o, Persistent) and o not in [self.__root, self.stringRegistry, self.pickledTypeList] ]
        if suspects: 
            LOG.warning('The following proxy objects are probably part of a reference cycle: \n{}'
                        .format(suspects))
        gc.collect()
        suspects = [o for o in gc.get_referrers(self) 
            if isinstance(o, Persistent) and o not in [self.__root, self.stringRegistry, self.pickledTypeList] ]
        if suspects:
            raise ValueError("Cannot close {} - some proxies are still around: {}"
                             .format(self, ' '.join([repr(s) for s in suspects])))
        self.__flush()
...
#         LOG.debug( "allocated: {origFreeOffset}, {size}, {newFreeOffset}".format(**locals()))
    ...
    property root:
        def __get__(self):
            ...
            if self.__root is None:
                LOG.debug('self.createTypes: {self.createTypes}'.format(self=self))
                if self.createTypes:
                    for ptype in self.typeList:
                        if ptype.__name__ not in ('String', 'UInt', 'Float', ):#'__ListOfStrings'
                            x = ptype.reduce()
#                             LOG.debug( 'pickle data:'+ repr(x))
                ...
                try: Root = self.schema.Root
                except AttributeError:
                    raise Exception("The schema contains no type called 'Root'.")
                LOG.debug( 'self.p2FileHeader.o2Root #1: {0}'.format(hex(self.p2FileHeader.o2Root)) )
                ...
                LOG.debug( 'self.p2FileHeader.o2Root #2: {0}'.format(hex(self.p2FileHeader.o2Root)) )
            return self.__root

    def defineType(Storage storage, TypeDescriptor typeDescriptor):
        cdef:
            type meta = typeDescriptor.meta
        typeDescriptor.verifyTypeParameters(typeDescriptor.typeParameters) # need to check if they are filled in
        return meta.typedef(storage, typeDescriptor.className, typeDescriptor.proxyClass, *typeDescriptor.typeParameters)
    ...
    cpdef object internValue(Storage self, str typ, value):
        if typ == 'time':
            ...
            return value
        else: raise TypeError(str(typ))            
        
    def populateSchema(self): pass
