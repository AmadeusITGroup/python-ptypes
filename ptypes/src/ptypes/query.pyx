    ...
    cpdef Structure getAggregate(self, dict dick):
        ...
        for keyField in groupClass.pfields: 
            try:
                ...
                e.args = e.args[0] + "Candidate fields are " + groupClass.pfields, 
                raise
            d[keyField.name] = key
    ...
    def rollup(self):
        ...
        for keyField in groupClass.pfields:
            for group, aggregate in self.iteritems():
                if keyField.get(group).contents == 'total': continue
                volatileKeyOfTotal = group.contents._replace({keyField.name:'total'})
                ...
                for aggregateField in aggregateClass.pfields:  
                    ...
                    aggregateField.get(totalAggregate).add(aggregateField.get(aggregate).contents)  
    
    def entropy(PHashTable self, str aggregateName, str keyName1, str keyName2):
        ...
        cdef:
            ...
            Structure   key, value
            PField      keyField
            PField      aggregateField   = getattr(aggregateClass, aggregateName) 
            dict        d
            PHashEntry  entry
            double      sum12 = 0, sum1 = 0, sum2 = 0, N=-1, natPerBit = logarithm(2) 
            double      n, H1, H2, H12
            tuple       non_total_fields = (keyName1, keyName2)
            list        total_fields = list()

        # In any field other than total_fields (i.e. keyName1 and keyName2) we do 
        # not find 'total' then we have to skip the entry.
        # If none of keyName1 and keyName2 are 'total', then we add the cell into 
        # the running sum of the entropy of the joint distribution.
        # If keyName1 is 'total', then we add the cell into the running sum of the entropy 
        # of the marginal distribution of key2 (and vice versa).  
        #  
        for keyField in groupClass.pfields:
        ...
        for group, aggregate in self.iteritems():
            d = group.contents
            if any(keyField.get(group).contents != 'total' for keyField in total_fields):
                continue
...
######################  Query Interface ########################        
variableOrdinal = count(0)  # XXX The GIL is here used a lock for the ordinal?
...
cdef class BindingRule(Variable):
    ...
    def __init__(self, *usedBindingRules, bint shortCut=False): # TODO: remove *usedBindingRules
        Variable.__init__(self)
#         print 'usedBindingRules', self.name, usedBindingRules
        self.prevBindingRule = self.nextBindingRule = None
        self.shortCut = shortCut
        self.keyField = None #self.resultField = 
        self.usedBindingRules = set(usedBindingRule for usedBindingRule in usedBindingRules if usedBindingRule )
    ...
    # - In Query definitions, a RichCompare object needs to be returned, postponing evaluation
    #      until query-runtime, and comparing the values assigned to the bindingRule in that iteration
    # - When the bindingRule chain is assembled, we must be able to compare bindingRule objects for identity 
    #       (== and !=) immediately  
...
#         return RichCompare(self, other, op)
    ...
    cdef getAll(BindingRule self, query, QueryContext result):
        """ This method must be overridden in derived classes.
            BindingRules that may be first bindingRules (e.g. Each and Param) should define it as
            cpdef, so that it is callable from Python as well. For other bindingRules,
            cdef is enough.
        """ 
...
cdef class SubQuery(BindingRule):
    ...
    def __init__(self, BindingRule firstBindingRule, BindingRule predicate, bint shortCut=False):
        BindingRule.__init__(self, firstBindingRule, predicate, shortCut=shortCut)
...
cdef class RichCompare(BindingRule):
    ...
    cdef:
        ...
        object other
        int op
    def __init__(RichCompare self, BindingRule oneBindingRule, other, ComparisonOperator op, bint shortCut=False):
        if isinstance(other, BindingRule):
            BindingRule.__init__(self, oneBindingRule, other, shortCut=shortCut)
        else:
    ...
    cdef getAll(RichCompare self, query, QueryContext result):
        ...
        if self.op==2: rv = oneValue == otherValue
        elif self.op==3: rv = oneValue != otherValue
        else:
            assert False, self.op # only == and != are supported
#         if op==0: return self.offset  < other.offset
#         if op==1: return self.offset <= other.offset
#         if op==4: return self.offset  > other.offset
#         if op==5: return self.offset >= other.offset
        result.setattr(self.name, rv)
        self.getAllRecursively(query, result)
        
# cpdef RichCompare LessThan       (BindingRule oneBindingRule, other, shortCut): return RichCompare(oneBindingRule, other, LT, shortCut)
# cpdef RichCompare LessOrEqual    (BindingRule oneBindingRule, other, shortCut): return RichCompare(oneBindingRule, other, LE, shortCut)
cpdef RichCompare Equal          (BindingRule oneBindingRule, other, shortCut): return RichCompare(oneBindingRule, other, EQ, shortCut)
cpdef RichCompare NotEqual       (BindingRule oneBindingRule, other, shortCut): return RichCompare(oneBindingRule, other, NE, shortCut)
# cpdef RichCompare GreaterThan    (BindingRule oneBindingRule, other, shortCut): return RichCompare(oneBindingRule, other, GT, shortCut)
# cpdef RichCompare GreaterOrEqueal(BindingRule oneBindingRule, other, shortCut): return RichCompare(oneBindingRule, other, GE, shortCut)
...
cdef class Constant(BindingRule): 
    ...
    def __init__(self, value, typ=None, bint shortCut=False):
        ...
        self.value = value
        self.typ = ("string" if isinstance(value, str) else None) if typ is None else typ 
    
    cpdef getAll(self, query, QueryContext result):
        try: value = query.storage.internValue(self.typ, self.value )
        except ValueError as e:
            e.args = ("Could not intern the value for constant '{0}'".format(self.name),) + e.args
            raise
#         print 'constant', self.name, self.typ, type(value), value
...
cdef class Param(BindingRule): 
    ...
    def __init__(self, typ='string', name=None, description="", bint shortCut=False):
        BindingRule.__init__(self, shortCut=shortCut)
        self.name = name
        self.typ = typ
    ...
    cpdef getAll(self, query, QueryContext result):
        try: value = query.storage.internValue(self.typ, query.paramValues[self.name] )
        except ValueError as e:
            e.args = ("Could not intern the value for parameter '{0}'".format(self.name),) + e.args
            raise
        result.setattr(self.name, value)
        self.getAllRecursively(query, result)
...
cdef class Attribute(BindingRule):
    ...
    def __init__(self, BindingRule bindingRule, str attributeName=None, bint shortCut=False):
    ...
    cdef getAll(self, query, QueryContext result):
        ...
        try:
            attributeValue = getattr(persistent, self.attributeName or self.name)
            result.setattr(self.name, attributeValue)
        except Exception as e:
            e.args = e.args[0] + " while accessing attribute '{0}' of {1} in bindingRule '{2}'".format(self.attributeName, persistent, self.name),
            raise
        self.getAllRecursively(query, result)
...
cdef class Contents(BindingRule):
    ...
    cdef getAll(self, query, QueryContext result):
        ...
        try:
            ...
            e.args = e.args[0] + " while accessing the contents of {0} in bindingRule '{1}'".format( persistent, self.name),
            raise
        self.getAllRecursively(query, result)
...
cdef class QueryContext(object):
    ...
    def __init__(self, query=None, callback=None): 
        self.query = query
#         print 'self.query.aggregators', self.query.GroupBy, self.query.aggregators
        self.callback = callback if callback else noOp()
        self.dick = dict()
     
    def __repr__(self):
        return ', '.join(list(('{0} = {1}'.format(k, self.dick[k]) 
                               for k in sorted(self.dick) if not k.startswith('_')
                               )))
...
#         for k, v in other.dick.items():
            self.dick[aggregator.name] = aggregator.aggregateField.get(aggregate)
        for keyBindingRule in self.query.GroupBy:
            self.dick[keyBindingRule.name] = keyBindingRule.keyField.get(group)
...
cdef class AggregatingQueryContext(QueryContext):
    ...
    cdef do(self):
        cdef:
            Structure aggregate = self.contingencyTable.getAggregate(self.dick)
...
#         self.distinct = kwargs.get('distinct', False)
        ...
        self.memory = defaultdict(set) # { result: set() of seen rollingQueryContexts }

    def reset(self, result):
        result.dick[self.name] = self.accumulatorClass()
        
    cdef aggregate(self, Structure aggregate, AggregatingQueryContext aggregatingQueryContext):
        return self.doAggregate(aggregate, aggregatingQueryContext)
    
    cdef doAggregate(self, Structure aggregate, AggregatingQueryContext aggregatingQueryContext):
        raise NotImplementedError 
    
# cdef class Distinct(object):
#     cdef aggregate(self, Structure aggregate, AggregatingQueryContext aggregatingQueryContext): 
#         rolledUpQueryContextState = aggregate.saveState()
#         rollingQueryContextState = tuple(getattr(aggregatingQueryContext, (<BindingRule>bindingRule).name) for bindingRule in self.bindingRules)
...
#     
#     cdef doAggregate(self, Structure aggregate, AggregatingQueryContext aggregatingQueryContext):  
#         value = tuple(getattr(aggregatingQueryContext, (<BindingRule>bindingRule).name) for bindingRule in self.bindingRules) # XXX optimize
...
cdef class Count(AggregatorOf1):
    ...
    cdef doAggregate(self, Structure aggregate, AggregatingQueryContext aggregatingQueryContext):
#         print self, self.aggregateField
        self.aggregateField.get(aggregate).inc()  # XXX .inc is dynamically resolved
...
cdef class Sum(AggregatorOf1):
    ...
    cdef doAggregate(self, Structure aggregate, AggregatingQueryContext aggregatingQueryContext):  
        self.bindingRule.aggregateField.get(aggregate).add( aggregatingQueryContext.dick[self.bindingRule.name] )
...
class QueryMeta(type):
    def __new__(meta, class_name, bases, attribute_dict): #@NoSelf
        ...
        attribute_dict['GroupBy'       ] = GroupBy # make sure it is in there
        ...
        for k, v in attribute_dict.items():
            ...
            if isinstance(v, (Param ,)): 
                ...
                parameters.append(v)
            if isinstance(v, (LookUp,)): lookups.add(k)
            if isinstance(v, (Aggregator,)) : aggregators.add(v)
        ...
        if GroupBy:
            ...
            assert output2.createTypes # we only support blank storages for now
            ...
            for bindingRule in GroupBy:
                bindingRule.keyField =<PField?> getattr(GroupClass, bindingRule.name) 
            ...
            AggregateClass = StructureMeta('Aggregate', (Structure,), attributes)
            for aggregator in aggregators:
                aggregator.aggregateField =<PField?> getattr(AggregateClass, aggregator.name)
#                 print aggregator, aggregator.aggregateField 
            
            _                = HashEntryMeta.typedef(output2, 'ContingencyCell' , PHashEntry, 'Group', 'Aggregate') 
            ContingencyTable = HashTableMeta.typedef(output2, 'ContingencyTable', PConTable, 'ContingencyCell')
    ...
    def __orderBindingRules(klass, startPoints):  # @NoSelf #startPoints
        ...
        klass.firstBindingRule = klass.bindingRules[0] if klass.bindingRules else None
        ...
        for i in range(numberOfBindingRules):
            ...
            bindingRule.prevBindingRule = klass.bindingRules[i-1] if i   > 0 else None 
            bindingRule.nextBindingRule = klass.bindingRules[i+1] if i+1 < numberOfBindingRules else None 

class Query(object):
    """ Selects combinations of persistent objects and sends each combination to a generator.
    
        Treat this as an abstract base class and define your queries in subclasses derived from this.
        Specify the binding rules in the body of the class and override 
    """
    ...
    def __init__(self, Storage storage, **paramValues):
        ...
        if unInitializedParameters:
            raise TypeError('Uninitialised query parameters: {p}'.format(p=' ,'.join(sorted(unInitializedParameters))))
        if unusedParameters:
            raise TypeError('Unused query parameters: {p}'.format(p=' ,'.join(sorted(unusedParameters))))
        self.paramValues = paramValues
#         self.groups = dict()
            
    def __call__(self, QueryContext context=None, callback=None, unsigned long numCellsInContingencyTable=10000):
        """ Enumerate the object-combinations matching the binding rules and send every combination to a generator.
        
            @param context: An optional QueryContext instance that will be used for the enumaration & callbacks.
                            If missing, a QueryContext instance will be internally created and used.
            @param callback: An optional generator function to which the combinations will be sent.
                            If missing, self.processOne is going to be used. 
        """
        if callback is None:
        ...
        context.query = self
        if self.GroupBy:
            self.contingencyTable = self.ContingencyTable( numCellsInContingencyTable)
            runningQueryContext = AggregatingQueryContext(self, self.contingencyTable)
        else:
        ...
        if self.GroupBy:
            ...
            for entry in self.contingencyTable.iterEntries():
                ...
                context.do()
            context.end()
# why do we want to return the context object here?
#         return context 

    def processOne(self):
        """ A generator to which the query context is sent whenever it reaches a state matching all the binding rules.
        
            Can be overridden in derived classes. The default implementation prints a header and trailer 
