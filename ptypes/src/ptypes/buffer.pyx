...
cdef extern from "Python.h":
    int PyBUF_FULL_RO, PyBUF_C_CONTIGUOUS, PyBUF_ANY_CONTIGUOUS, PyBUF_FORMAT, PyBUF_SIMPLE #PyBUF_INDIRECT, PyBUF_STRIDES, PyBUF_ND
    int PyObject_CheckBuffer(object obj)
    int PyObject_GetBuffer  (object  obj, Py_buffer *view, int flags) except -1
    int PyBuffer_ToContiguous(void *buf, Py_buffer *view, Py_ssize_t len, char fort) except -1
    int PyBuffer_FillInfo(Py_buffer *view, object obj, void *buf, Py_ssize_t len, int readonly, int infoflags) except -1
#     Py_ssize_t PyBuffer_SizeFromFormat(const char *) http://bugs.python.org/issue15913
...
############################# Persistent Buffers #################################
# Objective:
#     be able to store object of any type that 
#        1) supports the buffer protocol (https://docs.python.org/2/c-api/buffer.html)
...
# Persistent version of https://github.com/python/cpython/blob/master/Include/object.h#L178-191
cdef:
    struct CBuffer:
        Offset   o2buf # void *
        Py_ssize_t len # The total length of the memory in bytes = ((*shape)[0] * ... * (*shape)[ndims-1])*itemsize
        Py_ssize_t itemsize  # This is Py_ssize_t so it can be pointed to by strides in simple case
        int ndim
        Offset o2format # char *
        Offset o2shape  # Py_ssize_t *
        Offset o2strides # Py_ssize_t *
        
        #int readonly - we always provide R/W access
        # 
        # char   fortran='C' - We will always store stuff C-contiguously         
        # 
        # Offset o2suboffsets # Py_ssize_t * - We do not support suboffsets (we always set it to NULL)
        #
        #view.internal is not for the consumer 
    
cdef class BufferMeta(PersistentMeta):
    """ @param reconstructor: Python code that leaves in the local name space a reference to a callable
                                under the name 'reconstruct' when it is executed
    """
    cdef:
        object  reconstructor # 
        object  reconstruct   # a callable that can re-create the instance of the object from a copy of its buffer
    def __init__(ptype, 
                       Storage       storage, 
                       str           className, 
                       type          proxyClass,
...
#         exec (reconstructor, globalNameSpace, localNameSpace)
        ...
        PersistentMeta.__init__(ptype, storage, className, proxyClass, sizeof(CBuffer))
...
cdef class PBuffer(AssignedByReference): 
    ...
    cdef inline CBuffer *getP2IS (PBuffer self): return <CBuffer *>self.p2InternalStructure 
    ...
    def __init__(self, object value): 
        ...
        # PyBUF_FULL_RO = PyBUF_FORMAT  | PyBUF_INDIRECT (which implies PyBUF_STRIDES and indirectly PyBUF_ND)
...
#                 view.strides[i-1] = view.strides[i] * view.shape[i]            
        ...
        if view.format is NULL:            # PyBUF_FORMAT | PyBUF_FULL_RO | PyBUF_INDIRECT | PyBUF_STRIDES | PyBUF_ND
            PyBuffer_FillInfo(&view, value, view.buf, view.len, 1, PyBUF_FULL_RO ) 
        cBuffer.o2format = self.allocateAndCopy(view.format, strlen(view.format)+1)
...
#             cBuffer.itemsize = PyBuffer_SizeFromFormat(view.format)
        ...
        LOG.debug("__init__ {} {} {}".format(cBuffer.o2format, cBuffer.itemsize, view.ndim)) 
        
        if view.ndim == 0:
            cBuffer.o2shape = cBuffer.o2strides = 0 # cBuffer->o2suboffsets = 
        else:  
            ...
            # cBuffer.o2suboffsets = self.allocateAndCopy(view.suboffsets, length)             
    ...
    def __getbuffer__(self, Py_buffer* view, int flags):
        # We always procide a writable, C-contiguous direct buffer.
        # 
        ...
        cdef bint isCContiguousOk = (flags == PyBUF_SIMPLE) or flags & (PyBUF_C_CONTIGUOUS | PyBUF_ANY_CONTIGUOUS) 
        if not isCContiguousOk:
            raise ValueError("Persistent buffers are C-contiguous; you requested a non-C-contiguous buffer (flags={})."
                             .format(hex(flags)))
        ...
        view.strides = <Py_ssize_t*> self.ptype.offset2Address(cBuffer.o2strides)
    ...
    def __releasebuffer__(self, Py_buffer* info):
        ...
        # not needed; __getbuffer__ never allocates memory, so there is no need to release it
        pass
